...
import utilStyles from "../../styles/educationSwipeArea.module.css"
import dynamic from 'next/dynamic'

const ModalVideo = dynamic(() => import('../atoms/modalVideo'))
const ContentsBlock = dynamic(() => import("../atoms/contensBlock"))

export default function EduSwipeArea({listedContents, playingInView}) {
    ...
    const [ swiperLoop, setSwiperLoop ] = useState(true)
    ...
    const onTouchEnd = (clickedIndexState, listedContents) => {
        ...
        if (isRightSwipe){
            if (clickedIndexState > 0) {
                setModalImageState(listedContents[clickedIndexState - 1].mediaURL)
                ...
                setModalImageState(listedContents[contentsLength - 1].mediaURL)
            ...
            if (clickedIndexState < contentsLength - 1){
                ...
                setModalImageState(listedContents[0].mediaURL)
    ...
    if (modalMediaTypeState === "Image") {
        ...
        modalContents = <div className={utilStyles.modalContentsTouchSwipingWrapper}
        onTouchStart={onTouchStart}
        onTouchMove={onTouchMove}
        onTouchEnd={() => onTouchEnd(clickedIndexState, listedContents)}>
            <ModalVideo
        ...
        {listedContents.map((contents, index) => {
            return(
                <SwiperSlide className={`${utilStyles.positionRelative} ${utilStyles.brandArtistResponsiveWidth} ${utilStyles.swiperFlexibleSize}`} key={index} onClick={() => {
                    ...
                    setModalImageState(contents.mediaURL)
                    ...
                    <ContentsBlock 
                        ...
                        ontentsType={contents.mediaType}  
                        contentsLabel="" 
                        contentsImgUrl={contents.mediaURL} 
                        contentsDescription="" 
